...
export class ConstruccService {
	...
	): Observable<PaginatedResponse<ConstruccProveedor>> {
		const params = this.buildParams(filters);

		return this.http
			.get<PaginatedResponse<ConstruccProveedor>>(
	...
	): Observable<PaginatedResponse<ConstruccProducto>> {
		const params = this.buildParams(filters);

		return this.http
			.get<PaginatedResponse<ConstruccProducto>>(
	...
	getFiltrosProductos(): Observable<ApiResponse<any>> {
		return this.http
			.get<ApiResponse<any>>(`${this.baseUrl}/productos/filtros`, {
	...
	getSugerenciasProductos(
		...
		proveedorId?: number,
		limite = 10
	): Observable<ApiResponse<{ sugerencias: SugerenciaProducto[] }>> {
		let params = new HttpParams()
			.set("termino", termino)
		...
		return this.http
			.get<ApiResponse<{ sugerencias: SugerenciaProducto[] }>>(
	...
	getMarcasProveedor(
		proveedorId: number
	): Observable<ApiResponse<{ marcas: ConstruccMarca[]; total: number }>> {
		return this.http
			.get<ApiResponse<{ marcas: ConstruccMarca[]; total: number }>>(
	...
	getCategoriasProveedor(
		...
		ApiResponse<{ categorias: ConstruccCategoria[]; total: number }>
	> {
		let params = new HttpParams();
		...
		return this.http
			.get<ApiResponse<{ categorias: ConstruccCategoria[]; total: number }>>(
	...
	getLineasProveedor(
		proveedorId: number
	): Observable<ApiResponse<{ lineas: any[]; total: number }>> {
		return this.http
			.get<ApiResponse<{ lineas: any[]; total: number }>>(
	 ...
	 */
	getUnidadesProveedor(
		proveedorId: number
	): Observable<ApiResponse<{ unidades: ConstruccUnidad[]; total: number }>> {
		return this.http
			.get<ApiResponse<{ unidades: ConstruccUnidad[]; total: number }>>(
	 ...
	 */
	getCatalogosCompletos(proveedorId: number): Observable<
		ApiResponse<{
	...
	getEstadisticas(): Observable<ApiResponse<any>> {
		return this.http
			.get<ApiResponse<any>>(`${this.baseUrl}/reportes/estadisticas`, {
				headers: this.getHeaders(),
			})
			.pipe(retry(1), catchError(this.handleError));
	}
	...
	getFiltrosDisponibles(): Observable<ApiResponse<any>> {
		return this.http
			.get<ApiResponse<any>>(`${this.baseUrl}/config/filtros-disponibles`, {
				headers: this.getHeaders(),
			})
			.pipe(retry(1), catchError(this.handleError));
	}
	...
	getOpcionesOrdenamiento(): Observable<ApiResponse<any>> {
		return this.http
			.get<ApiResponse<any>>(`${this.baseUrl}/config/opciones-ordenamiento`, {
				headers: this.getHeaders(),
			})
			.pipe(retry(1), catchError(this.handleError));
	}
...
/**
 * Metodos para probar el funcionamiento de los endpoinst definidos en el servicio
 */
class TestService {
	static testService() {
		// =============================
		...
		// =============================
		this.construccService.getProveedores({}).subscribe({
			next: (res) => console.log("getProveedores:", res),
			error: (err) => console.error("Error getProveedores:", err),
		});

		this.construccService.buscarProveedores({}).subscribe({
			next: (res) => console.log("buscarProveedores:", res),
			error: (err) => console.error("Error buscarProveedores:", err),
		});

		// =============================
		// 2️⃣ Productos por proveedor
		// =============================
		const proveedorId = 1;
		this.construccService.getProductosProveedor(proveedorId, {}).subscribe({
			next: (res) => console.log("getProductosProveedor:", res),
			error: (err) => console.error("Error getProductosProveedor:", err),
		});

		this.construccService.buscarProductosProveedor(proveedorId, {}).subscribe({
			next: (res) => console.log("buscarProductosProveedor:", res),
			error: (err) => console.error("Error buscarProductosProveedor:", err),
		});

		// =============================
		// 3️⃣ Productos generales
		// =============================
		this.construccService.buscarProductos({}).subscribe({
			next: (res) => console.log("buscarProductos:", res),
			error: (err) => console.error("Error buscarProductos:", err),
		});

		this.construccService.getFiltrosProductos().subscribe({
			next: (res) => console.log("getFiltrosProductos:", res),
			error: (err) => console.error("Error getFiltrosProductos:", err),
		});

		this.construccService
			.getSugerenciasProductos("cemento", proveedorId, 5)
			.subscribe({
				next: (res) => console.log("getSugerenciasProductos:", res),
				error: (err) => console.error("Error getSugerenciasProductos:", err),
			});

		// =============================
		// 4️⃣ Catálogos
		// =============================
		this.construccService.getMarcasProveedor(proveedorId).subscribe({
			next: (res) => console.log("getMarcasProveedor:", res),
			error: (err) => console.error("Error getMarcasProveedor:", err),
		});

		this.construccService.getCategoriasProveedor(proveedorId).subscribe({
			next: (res) => console.log("getCategoriasProveedor:", res),
			error: (err) => console.error("Error getCategoriasProveedor:", err),
		});

		this.construccService.getLineasProveedor(proveedorId).subscribe({
			next: (res) => console.log("getLineasProveedor:", res),
			error: (err) => console.error("Error getLineasProveedor:", err),
		});

		this.construccService.getUnidadesProveedor(proveedorId).subscribe({
			next: (res) => console.log("getUnidadesProveedor:", res),
			error: (err) => console.error("Error getUnidadesProveedor:", err),
		});

		this.construccService.getCatalogosCompletos(proveedorId).subscribe({
			next: (res) => console.log("getCatalogosCompletos:", res),
			error: (err) => console.error("Error getCatalogosCompletos:", err),
		});

		// =============================
		// 5️⃣ Estadísticas y reportes
		// =============================
		this.construccService.getEstadisticas().subscribe({
			next: (res) => console.log("getEstadisticas:", res),
			error: (err) => console.error("Error getEstadisticas:", err),
		});

		this.construccService.getResumenProveedor(proveedorId).subscribe({
			next: (res) => console.log("getResumenProveedor:", res),
			error: (err) => console.error("Error getResumenProveedor:", err),
		});

		// =============================
		// 6️⃣ Configuración
		// =============================
		this.construccService.getFiltrosDisponibles().subscribe({
			next: (res) => console.log("getFiltrosDisponibles:", res),
			error: (err) => console.error("Error getFiltrosDisponibles:", err),
		});

		this.construccService.getOpcionesOrdenamiento().subscribe({
			next: (res) => console.log("getOpcionesOrdenamiento:", res),
			error: (err) => console.error("Error getOpcionesOrdenamiento:", err),
		});
	}
}
