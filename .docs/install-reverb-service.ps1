# Script para instalar Reverb como servicio de Windows
# Ejecutar como Administrador

$serviceName = "LaravelReverb"
...
nssm install $serviceName $phpPath $artisanPath reverb:start
...
nssm set $serviceName AppStdout "$workingDir\storage\logs\reverb-stdout.log"
nssm set $serviceName AppStderr "$workingDir\storage\logs\reverb-stderr.log"

# Configurar descripción
Write-Host "📋 Configurando descripción..." -ForegroundColor Cyan
nssm set $serviceName Description "Laravel Reverb WebSocket Server"
...
if ($service.Status -eq "Running") {
    ...
    Write-Host "   - $workingDir\storage\logs\reverb-stdout.log"
    Write-Host "   - $workingDir\storage\logs\reverb-stderr.log"
} else {
